...
export async function getUpdatePreview(): Promise<UpdatePreview> {
  ...
  // package-lock.json is auto-generated by npm and commonly drifts after installs.
  // It is never a meaningful local edit so we exclude it from the dirty-file check.
  const dirtyFiles = splitLines(await git(["status", "--porcelain"])).filter(
    (line) => !line.endsWith("package-lock.json"),
  );
...
export async function pullUpdate(preview: UpdatePreview): Promise<PullUpdateResult> {
  ...
  // Auto-restore package-lock.json if it's dirty — it's generated by npm and
  // will block git pull otherwise. The pull will regenerate it correctly.
  const allDirty = splitLines(await git(["status", "--porcelain"]));
  if (allDirty.some((line) => line.endsWith("package-lock.json"))) {
    await git(["restore", "package-lock.json"]).catch(() => {});
  }
